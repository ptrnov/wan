    ...
    <?= GridView::widget([
        ...
        'columns' => [
			...
			['class' => 'yii\grid\ActionColumn',
				...
				if ($action === 'view')
        {
          	$url = Yii::$app->urlManager->createUrl('option?id='.$key['MODUL_ID'].'');
        }

				else if ($action === 'update')
								$url = Yii::$app->urlManager->createUrl('question/update?id='.$key['MODUL_ID'].'');
				else if ($action === 'delete')
								$url = Yii::$app->urlManager->createUrl('question/delete?id='.$key['MODUL_ID'].'');
						return $url;
				},
